...
2. legal_metrology_compliance: an object with one key per mandatory declaration (manufacturer_info, generic_name, net_quantity, manufacture_date, best_before_use_by, mrp, unit_sale_price, consumer_care, country_of_origin, fssai_license, dimensions_if_applicable). For each: { present: boolean, value: string|null, compliant: boolean, issue: string|null, note: string|null, status: "present"|"ok_inferred"|"missing"|"not_visible"|"not_applicable" }. Use "not_visible" per the ANTI-HALLUCINATION rules — only use "missing" when the label is legible and the declaration is truly absent. For a declaration not applicable to the ACTIVE CATEGORY set present:false, compliant:true, status:"not_applicable", issue:"Not applicable to <category> products". "note" is optional neutral text (NOT a problem) — leave it null except where a rule below tells you to set it.

COUNTRY OF ORIGIN — status rules (base the decision ONLY on the visible label text; do NOT use outside knowledge to guess a country):
- The label explicitly states "Made in <country>", "Country of Origin: <country>", "Product of <country>", "Imported from <country>" or similar => status: "present", present: true, compliant: true, value: the country stated.
- No explicit country-of-origin statement, BUT the manufacturer / packer / marketer address on the label is a full Indian address (an Indian state name AND a 6-digit PIN code, or an unambiguous Indian city) => status: "ok_inferred", present: false, compliant: true, value: "India (inferred from manufacturer address)", note: "Manufacturer address indicates India but there is no explicit country-of-origin declaration. Legal Metrology requires an explicit declaration for imported goods only."
- No explicit country-of-origin statement AND the address is foreign, or is too partial to place in India => status: "missing", present: false, compliant: false, issue: "No country-of-origin declaration and the manufacturer address does not establish India — mandatory for imported goods." For a clearly foreign address this IS a violation.
- The manufacturer / packer address is not in the extracted text at all => status: "not_visible", present: false, compliant: false.
